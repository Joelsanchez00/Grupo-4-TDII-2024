...
int main(void)
{
  ...
  const char *msg_asc = "Flanco Ascendente Detectado\r\n";
  const char *msg_desc = "Flanco Descendente Detectado\r\n";
  /* USER CODE END 1 */
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  if (currentButtonState && !buttonState)
	  {
		  ...
		  HAL_UART_Transmit(&huart3, (uint8_t*)msg_asc, strlen(msg_asc), HAL_MAX_DELAY);
	      // Aquí puedes agregar la lógica para mostrar el mensaje en tu pantalla o LED
	      secuenciaActual = (secuenciaActual + 1) % numeroTotalSecuencias;
	  }

	  // Detecta el flanco descendente
	  else if (!currentButtonState && buttonState) {
		  HAL_UART_Transmit(&huart3, (uint8_t*)msg_desc, strlen(msg_desc), HAL_MAX_DELAY);
	      // Aquí puedes agregar la lógica para mostrar el mensaje en tu pantalla o LED
