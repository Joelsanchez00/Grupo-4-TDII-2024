...
const uint32_t LED[] = {LD1_Pin , LD2_Pin , LD3_Pin};
const uint32_t BOTON = USER_Btn_Pin;
...
void BOTONCTR (){
	...
	bool BOTONCTR = HAL_GPIO_ReadPin(GPIOC, USER_Btn_Pin) == GPIO_PIN_RESET;
	    if (BOTONCTR && !botonPresionado && HAL_GetTick() - tiempoUltimoCambio > 100) {
...
{
	static int ledActual = LedCantidad - 1;

	    LEDON(LED[ledActual]);
	    HAL_Delay(200);
	    LEDOFF(LED[ledActual]);

	    ledActual = (ledActual - 1 + LedCantidad) % LedCantidad;
	    HAL_Delay(200);
}
